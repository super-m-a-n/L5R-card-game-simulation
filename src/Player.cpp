...
Player::Player(const std::string &name){
	...
	money = dynamic_cast<Stronghold*>(holdings[0])->initialMoney();
	//money = holdings[0]->initialMoney();
	roundMoney = money;
	numberOfProvinces= NO_PROVINCES;

	for(unsigned int i=0; i< numberOfProvinces; i++){
...
unsigned int Player::give_no_provinces(){ return numberOfProvinces; }
...
void Player::decrease_arena(unsigned int pos){
	for(unsigned int i= pos+1; i< arena.size(); i++)
		arena[i]--;
}
...
void Player::battlePhase(Player *p){
	...
	if(p!= NULL){ //If the player has chosen battle
		...
		for(unsigned int i= 0; i< p->arena.size(); i++)
			points_defender+= p->army[p->arena[i]]->get_defense();

		int province_defense = dynamic_cast<Stronghold*>(p->holdings[0])->get_initialDefense();
		...
		if(points_attacker - points_defender > province_defense){ //Attacker won the battle, province is destroyed
			...
			if(points_attacker > points_defender){
				...
				int lost_points= 0; unsigned int k = 0;
				...
				int lost_points= 0; unsigned int k= 0;
				while(lost_points < (points_attacker - points_defender)){
			...
			for(unsigned i= 0; i< arena.size(); i++){
